...
public class RcCar implements RemoteControl {
	...
	final double INITIAL_BATTERY_CAPACITY = 500;		// RC카 초기 배터리 용량 : 500
	...
	public void setSpeed(double speed) {
		...
		if (!isOn) {
			System.out.println("RC카 전원을 켜주세요.");
		}
		else {
			// 배터리 잔량이 없는 상태에서는 속력 설정 X
			if (battery <= 0) {
				System.out.println("RC카 배터리를 교체해주세요.");
				this.speed = 0;
			}
			else {
				if (speed > RemoteControl.MAX_SPEED) {
					this.speed = RemoteControl.MAX_SPEED;
					System.out.println("RC카 최고 속력은 " + RemoteControl.MAX_SPEED + "입니다.");
				}
				else if (speed < RemoteControl.MIN_SPEED) {
					this.speed = RemoteControl.MIN_SPEED;
					System.out.println("RC카 최저 속력은 " + RemoteControl.MIN_SPEED + "입니다.");
				}
				else {
					this.speed = speed;
				}
				...
				if (this.speed > battery) {
					this.speed = battery;
					System.out.println("배터리 부족으로 RC카 속력이 감소하였습니다.");
				}
				System.out.println("RC카 속력 : " + this.speed);
	...
	public String toString() {
		return "RcCar [battery=" + battery + ", speed=" + speed + ", isOn=" + isOn + "]";
	}
